...
static void qca_hs_pm_suspend(struct device *dev)
{
...
static int qca_hs_pm_resume(struct device *dev)
{
	struct platform_device *pdev = to_platform_device(dev);
	struct msm_hs_port *msm_uport = get_matching_hs_port(pdev);
...
#ifdef CONFIG_PM
static int qca_hs_pm_sys_suspend_noirq(struct device *dev)
{
	struct platform_device *pdev = to_platform_device(dev);
	struct msm_hs_port *msm_uport = get_matching_hs_port(pdev);
...
static int qca_hs_pm_sys_resume_noirq(struct device *dev)
{
	struct platform_device *pdev = to_platform_device(dev);
	struct msm_hs_port *msm_uport = get_matching_hs_port(pdev);
...
static void qca_serial_hs_rt_init(struct uart_port *uport)
{
	struct msm_hs_port *msm_uport = UARTDM_TO_MSM(uport);
...
static int qca_hs_runtime_suspend(struct device *dev)
{
	qca_hs_pm_suspend(dev);
	return 0;
}

static int qca_hs_runtime_resume(struct device *dev)
{
	return qca_hs_pm_resume(dev);
}
#else
static void qca_serial_hs_rt_init(struct uart_port *uport) {}
static int qca_hs_runtime_suspend(struct device *dev) {}
...
static const struct dev_pm_ops qca_hs_dev_pm_ops = {
	.runtime_suspend = qca_hs_runtime_suspend,
	.runtime_resume = qca_hs_runtime_resume,
	.runtime_idle = NULL,
	.suspend_noirq = qca_hs_pm_sys_suspend_noirq,
	.resume_noirq = qca_hs_pm_sys_resume_noirq,
};
